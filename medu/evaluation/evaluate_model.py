...
from medu.datasets import get_loaders_from_dataset_and_unlearner_from_cfg_with_indices
import numpy as np

from medu.settings import default_evaluation_loaders
...
import medu.models
from medu.configurations import get_img_size_for_dataset, get_num_classes
from medu.datasets import get_loaders_from_dataset_and_unlearner_from_cfg

from medu.models import get_model
...
from medu.utils import DictConfig, setup_seed
...
def evaluate_model_on_loaders(
    model, retain_loader, forget_loader, val_loader, test_loader, random_state, device, log_file
):
    setup_seed(random_state)
    model.eval()
    model.to(device)
    ...
    # )
    # print(evaluation)
    msg = f"Evaluating on loaders of sizes: Retain {len(retain_loader.dataset)} | Forget {len(forget_loader.dataset)} | Val {len(val_loader.dataset)} | Test {len(test_loader.dataset)}"
    log_and_print(msg, log_file)
    # indices_msg = f"{retain_loader.dataset.indices} {forget_loader.dataset.indices} {val_loader.dataset.indices} {test_loader.dataset.indices}"
    # log_and_print(indices_msg, log_file)
    evaluation = evaluate_model(
        model, retain_loader, forget_loader, val_loader, test_loader, device=device
    )
    log_and_print(str(evaluation), log_file)
    return evaluation
...
class ModelEvaluationApp:
    ...
    def run_on_loaders(self, retain_loader, forget_loader, val_loader, test_loader, log_file):
...
def main(args):
    ...
    if args.split_ndx is None and args.forget_ndx is None:
        ...
        dataset_cfg = DictConfig({"name": "isic"})
        unlearner_cfg = DictConfig(

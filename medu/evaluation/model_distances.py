...
def models_normalized_l2_distance(model_a: Module, model_b: Module) -> float:
    ...
    if next(model_b.parameters()).device != device:
        model_b = model_b.to(device)
    
    distance_a = torch.tensor(0.0, device=device)
    for param in model_a.parameters():
        distance_a += torch.norm(param, p=2) ** 2
    norm_a = torch.sqrt(distance_a)  # GPU张量，不是CPU float
    
    distance_b = torch.tensor(0.0, device=device)
    for param in model_b.parameters():
        distance_b += torch.norm(param, p=2) ** 2
    norm_b = torch.sqrt(distance_b)  # GPU张量，不是CPU float
    ...
    return torch.sqrt(distance).item()  # # Convert to CPU float and return

...
class GRINV2Unlearner(BaseUnlearner):
    ...
    ORIGINAL_ALPHA = 0.01  # Perturbation scale factor (replaces noise_std)
    ...
    HYPER_PARAMETERS = {
        **medu.settings.HYPER_PARAMETERS,
        ...
        "unlearner.cfg.alpha": medu.settings.HP_FLOAT,  # Changed from noise_std to alpha
    ...
    def add_adversarial_perturbation(self, model: Module, mask, grad_diff, alpha):
        """
        Add adversarial perturbation to parameters selected by the mask.
        Perturbation: δ_i = α · ΔG_i = α · (G_f^(i) - G_r^(i))
        """
        with torch.no_grad():
            for i, param in enumerate(model.parameters()):
                if i < len(mask) and mask[i] is not None and i < len(grad_diff):
                    # Generate adversarial perturbation: δ = α · ΔG
                    perturbation = alpha * grad_diff[i]
                    # Apply perturbation only to selected parameters (where mask[i] = 1)
    ...
    def unlearn(
        ...
        """Main unlearning method implementing the GRIN algorithm with adversarial perturbations"""
        device = self.device
        selection_ratio = self.cfg.selection_ratio
        alpha = self.cfg.alpha  # Perturbation scale factor
        ...
        # Step 2: Compute gradient difference ΔG = G_f - G_r
        print("Calculating gradient difference (ΔG = G_f - G_r)...")
        grad_diff = []
        for gf, gr in zip(grad_f, grad_r):
            grad_diff.append(gf - gr)
        
        # Step 3: Compute influence scores (for parameter selection)
        ...
        # Step 5: Apply adversarial perturbation to selected parameters
        print("Applying adversarial perturbation to selected parameters...")
        model_unlearn = self.add_adversarial_perturbation(model_unlearn, mask, grad_diff, alpha)
        
        return model_unlearn


def grinv2_default_optimizer():
...
@dataclass
class DefaultGRINV2UnlearningConfig:
    """Default configuration for GRIN unlearning with adversarial perturbations"""
    num_epochs: int = GRINV2Unlearner.ORIGINAL_NUM_EPOCHS
    batch_size: int = GRINV2Unlearner.ORIGINAL_BATCH_SIZE
    selection_ratio: float = GRINV2Unlearner.ORIGINAL_SELECTION_RATIO
    alpha: float = GRINV2Unlearner.ORIGINAL_ALPHA  # Changed from noise_std to alpha
    epsilon_percentile: float = GRINV2Unlearner.ORIGINAL_EPSILON_PERCENTILE

    optimizer: typ.Dict[str, typ.Any] = field(
        default_factory=grinv2_default_optimizer
    )
